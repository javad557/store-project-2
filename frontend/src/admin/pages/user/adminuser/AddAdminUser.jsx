...
import { addAdmin } from "../../../services/user/adminService.js";
...
function AddAdminUser() {
    ...
    const [permissionsOptions, setPermissionsOptions] = useState([]); // تعریف شده در اینجا
    const [rolesOptions, setRolesOptions] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const navigate = useNavigate();
    const hasFetched = useRef(false);

    // استفاده از useMemo برای محاسبه isFormDirty
    ...
    // دریافت Permission‌ها و Role‌ها
    useEffect(() => {
        const fetchPermissionsAndRoles = async () => {
            ...
            try {
                ...
                if (!response.data?.permissions || !response.data?.roles) {
                    throw new Error("پاسخ API شامل data.permissions یا data.roles نیست");
                }
                setPermissionsOptions(
                    response.data.permissions.map((perm) => ({
                        value: perm.id,
                        label: perm.name,
                    }))
                );
                ...
                showError(`خطایی در دریافت Permission‌ها یا Role‌ها رخ داد: ${error.message}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response?.status === 422) {
                ...
                showError(error.response?.data?.error || "ایجاد کاربر ادمین با خطا مواجه شد");
            }
        } finally {
            setIsLoading(false);
        }
