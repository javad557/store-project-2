...
import { addAddress, editAddress, getCities, getMyAddress, getProvinces } from "../../services/user/customerUserService";
...
function EditAddress() {
  ...
  const {
    ...
    queryKey: ["address", id],
    queryFn: () => getMyAddress(id),
  });

  useEffect(() => {
    if (address && address.data?.data) {
      setFormData({
        province_id: address.data.data.province_id?.toString() || "",
        city_id: address.data.data.city_id?.toString() || "",
        address: address.data.data.address || "",
        postal_code: address.data.data.postal_code?.toString() || "",
        unit: address.data.data.unit || "",
        mobile: address.data.data.mobile?.toString() || "",
        no: address.data.data.no || "",
      });
    }
  }, [address]);
  ...
  const mutation = useMutation({
    mutationFn: (updatedData) => editAddress(id, updatedData),
    onSuccess: (response) => {
  ...
  const validateField = (name, value) => {
    switch (name) {
      case "province_id":
        if (!value) return "استان الزامی است";
        return null;
      case "city_id":
        if (!value) return "شهر الزامی است";
        return null;
      case "address":
      ...
      case "postal_code":
        ...
        return null;
      case "no":
        if (!value) return "پلاک الزامی است";
